...
const { div, textarea, text, script, domReady } = require("@saltcorn/markup/tags")

// CDN assets
const headers = [
  { css: "https://uicdn.toast.com/editor/latest/toastui-editor.min.css" },
  { script: "https://uicdn.toast.com/editor/latest/toastui-editor-all.min.js" }
]
...
const ToastUIMarkdownEditor = {
  ...
  run: (nm, v, attrs, cls) =>
  {
    ...
    const placeholder = attrs && attrs.placeholder ? String(attrs.placeholder) : ""
    const hardwrap = attrs && typeof attrs.hardwrap === "boolean" ? attrs.hardwrap : true
    ...
    const hardBreakFix = (md) =>
    {
      ...
      for(let i = 0; i < lines.length; i++)
      {
        ...
        if(inCode || t === "" || /^[#>\-+*]\s/.test(t) || /^\d+\.\s/.test(t) || /^\|/.test(t))
        {
          out.push(line)
          continue
        }
        ...
        const nextIsPlain = next !== "" && !/^[#>\-+*]\s/.test(next) && !/^\d+\.\s/.test(next) && !/^\|/.test(next)
    ...
    return div({ class: cls },
      ...
      textarea({ name: fieldName, id: taId, style: "display:none" }, text(v || "")),
      // visible editor
      div({ id: edId }),
      // init + sync
      script(domReady(`
